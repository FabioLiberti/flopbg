...
def main():
    ...
    nn = f"{global_epochs:02d}eg"
    mm = f"{local_epochs:02d}el"
    zz = f"{num_clients:02d}cl"
    ...
    try:
        ...
        # Salva i parametri di esecuzione
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(os.path.join(execution_dir, 'training.log')),
                logging.StreamHandler()
            ]
        )

        # Definisci il metodo di split dei dati
        ...
        # Inizializza report manager per il monitoraggio
        report_manager.configure_monitoring(
            execution_dir=execution_dir,
            metrics=realtime_metrics,
            client_data={
                'participation': client_participation_per_dataset,
                'statistics': client_statistics_per_dataset
            }
        )

        # Crea directory per i risultati dei dataset
        dataset_results_dir = os.path.join(execution_dir, 'dataset_results')
        os.makedirs(dataset_results_dir, exist_ok=True)

        # Log dell'inizio dell'esecuzione
        logging.info(f"Avvio esecuzione con {len(config['datasets'])} dataset")
        logging.info(f"Parametri configurati: {execution_parameters}")

        # Salva la configurazione iniziale per i report
        ...
        logging.error(f"Errore durante l'inizializzazione: {str(e)}", exc_info=True)
        raise
    ...
    for dataset_info in config['datasets']:
        ...
        try:
            ...
            # Crea directory specifica per il dataset
            dataset_dir = os.path.join(dataset_results_dir, dataset_name)
            os.makedirs(dataset_dir, exist_ok=True)
            
            # Carica e prepara i dati
            logging.info("Caricamento e preparazione dei dati...")
            ...
            # Setup dei client con le loro capacità
            logging.info("Configurazione dei client...")
            client_capabilities = {}
            client_ids = list(range(num_clients))
            random.shuffle(client_ids)  # Mescola gli ID dei client per assegnare casualmente le tipologie

            # Calcola la distribuzione dei client per tipo
            ...
            logging.info("Esecuzione training centralizzato...")
            ...
            # Aggiorna e Salva le metriche centralizzate per la dashboard in tempo reale
            ...
            logging.info("Avvio Federated Learning...")
            ...
            # Ciclo di addestramento federato - Inizializzazione metriche federate
            federated_metrics = {
                'loss': [],
                'accuracy': [],
                'precision': [],
...
#########################################
                ...
                for res in results:
                    if res is not None:
                        ...
                        valid_updates += 1
                        logging.info(f"Updates validi ricevuti: {valid_updates}/{len(active_clients)}")
                    else:
                ...
                # Aggiorna e salva le metriche in tempo reale
                ...
                # Salvataggio metriche e risultati per i report
                round_results = {
                ...
                save_results_for_reports(round_results, execution_dir)
                ...
                # Logging delle metriche su TensorBoard
                ...
                if test_loss < best_loss:
                    ...
                    server.global_model.save(os.path.join(
                        dataset_dir,
                        f'best_model_{dataset_name}.keras'
                    ))
                else:
                    patience_counter += 1
                ...
                server.global_model.save(os.path.join(
                    execution_dir, 
                    f'global_model_round_{round_num + 1}_{dataset_name}.keras'
                ))
            ...
            # Salvataggio risultati finali del dataset
            ...
            plt.figure()
            plt.plot(realtime_metrics['rounds'], realtime_metrics['federated_loss'], label='Federated Loss')
            plt.xlabel('Rounds')
            plt.ylabel('Loss')
            ...
            plt.figure()
            plt.plot(realtime_metrics['rounds'], realtime_metrics['federated_accuracy'], label='Federated Accuracy')
            plt.xlabel('Rounds')
            plt.ylabel('Accuracy')
    ...
    plot_overall_comparison(
        ...
        num_clients, 
        save_fig=True,
        filename_prefix=os.path.join(execution_dir, 'overall_comparison'), 
        show_fig=False
    )
    ...
    plot_centralized_vs_federated_comparison(
        ...
        num_clients, 
        save_fig=True,
        filename=os.path.join(execution_dir, 'centralized_vs_federated_comparison.png'), 
        show_fig=False
    )

    for dataset_name, dataset_results in all_results.items():
        plot_results(
            dataset_results['centralized'], 
            dataset_results['federated'], 
            dataset_name, 
            global_epochs, 
            local_epochs, 
            num_clients,
            save_fig=True, 
            filename_prefix=os.path.join(execution_dir, f'{dataset_name}_metrics_comparison'), 
            show_fig=False
        )
    ...
    print_and_plot_results_table(
        ...
        save_fig=True, fig_filename=os.path.join(execution_dir, 'results_table.png'), 
        show_fig=False
    )
    ...
    plot_execution_times(
        execution_times, 
        save_fig=True,
        fig_filename=os.path.join(execution_dir, 'execution_times.png'), 
        show_fig=False
    )
    
    
    ############ manca piccola parte di #Generazione report finale attraverso il report manager

    
    
    # Chiusura e pulizia
    try:
                # Ferma lo scheduler dei report
                report_manager.stop_scheduler()
                
                # Chiudi i writer di TensorBoard
                for handler in logging.root.handlers[:]:
                    handler.close()
                    logging.root.removeHandler(handler)
                
                # Crea un file di completamento
                with open(os.path.join(execution_dir, 'execution_completed'), 'w') as f:
                    f.write(end_time.strftime('%Y-%m-%d %H:%M:%S'))
                
                logging.info("Esecuzione completata con successo")
                
    except Exception as cleanup_error:
                logging.error(f"Errore durante la pulizia finale: {str(cleanup_error)}", exc_info=True)
